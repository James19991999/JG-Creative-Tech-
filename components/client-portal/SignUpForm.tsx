...
function friendlyError(error: unknown): string {
  ...
  if (
    ...
    return FIREBASE_ERROR_MESSAGES[code] ?? "Couldn't create your account. Please try again.";
  }
  return "Couldn't create your account. Please try again.";
}

export function SignUpForm() {
  const { signUp, configured } = useClientPortalAuth();
    ...
    return (
      <div className="bg-surface-container-lowest p-8 rounded-xl whisper-shadow max-w-md mx-auto">
        <p className="text-on-surface-variant text-sm leading-relaxed">
          ...
          up — see the portal setup notes in the README.
        </p>
      </div>
    );
  }
